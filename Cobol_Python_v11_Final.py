...
SOFTWARE_VERSION = "1.2.3"  # Corrigidos problemas de criptografia
...
class SSHClientGUI:
    ...
    def __init__(self, root):
        ...
        # ALTERAÇÃO SOLICITADA: TROCA DO RÓTULO PARA "Número do Romaneio ou Tela:"
    ...
    def load_admin_config(self):
        ...
        # ALTERAÇÃO: SENHAS PADRÃO DEFINIDAS COMO 'admin'
        default_config = {
            'admin_password': self.encrypt_data('admin'),
            'master_password': self.encrypt_data('admin'),  # Alterado para 'admin'
        ...
        if os.path.exists(self.admin_config_file):
            try:
                ...
                # Descriptografar senhas se possível
                if 'admin_password' in config:
                    decrypted = self.decrypt_data(config['admin_password'])
                    if decrypted != config['admin_password']:
                        config['admin_password'] = decrypted
                    else:
                        # Se falhou, redefinir para padrão
                        config['admin_password'] = 'admin'
                
                if 'master_password' in config:
                    decrypted = self.decrypt_data(config['master_password'])
                    if decrypted != config['master_password']:
                        config['master_password'] = decrypted
                    else:
                        config['master_password'] = 'admin'  # Alterado para 'admin'
    ...
    def save_admin_config(self, config):
        ...
        if 'master_password' in config_to_save:
            config_to_save['master_password'] = self.encrypt_data(config_to_save['master_password'])
    ...
    def show_admin_dialog(self):
        ...
        def check_password():
            ...
            if admin_type == "admin":
                ...
                stored_pass = self.admin_config.get('master_password', 'admin')  # Alterado para 'admin'
                if password == stored_pass:
                    # Esconder elementos de autenticação
                    auth_frame.pack_forget()
                    type_frame.pack_forget()
        ...
        def forgot_password():
            ...
            messagebox.showinfo(
                ...
                "Após excluir, as senhas voltarão aos valores padrão (admin/admin).",
                parent=self.root
            )
...
"""
    ...
    def show_help(self):
        ...
        instructions = (
            ...
            "   - Filtros permanentes são aplicados automaticamente\n\n"
...
""")
    ...
    # Forçar regeneração do arquivo de configuração se estiver corrompido
    ...
    if os.path.exists(config_path):
        try:
            ...
            # Se o arquivo estiver corrompido, removê-lo
